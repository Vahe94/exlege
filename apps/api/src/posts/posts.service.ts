...
export class PostsService {
  ...
  async create(user: AuthUser, input: UpsertPostInput) {
    try {
  ...
  async list(user: AuthUser, query: ListPostsQuery) {
    const where = {
      tenantId: user.tenantId,
  ...
  async update(user: AuthUser, id: string, input: Partial<UpsertPostInput>) {
  ...
  async publish(user: AuthUser, id: string, publish: boolean) {
    await this.getById(user, id);
  ...
  async publicList(tenantId: string, query: { type?: PostType; page: number; pageSize: number }) {
    const where = {
      tenantId,
  ...
  async publicGetBySlug(tenantId: string, slug: string) {
    const post = await this.prisma.post.findFirst({
